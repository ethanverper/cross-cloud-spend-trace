...
### 10. Databricks live verification — two real, diagnosed detours, both resolved

Databricks live verification hit two genuinely different problems in
sequence. Both were root-caused with evidence before any fix was proposed
or attempted — neither was a collector code bug.
...
This was Databricks' own API telling us the *type* of credential presented
wasn't valid for this API surface. **Confirmed root cause**: Ethan had
copied the token's short reference ID shown in the token-management list
(a UI element that looks like it could be the token) rather than the
actual secret string, which Databricks only ever displays once, in a
one-time reveal popup at creation. A freshly generated token, copied
correctly from that popup, authenticated immediately — confirmed via a
direct `GET /api/2.0/preview/scim/v2/Me` call returning a real, valid
identity (`ethan_enrique_01@hotmail.com`, `allow-cluster-create`
entitlement) before retrying the actual collector.

**Second detour, after auth was fixed**: `jobs/list` returned
`{"has_more": false}` and `clusters/list` returned `{}` — genuinely empty,
not an error. Root-caused (not assumed) by directly querying both
endpoints outside the collector and confirming the token itself was fully
valid and correctly scoped for Jobs/Clusters (a `workspace`-scope-only 403
on an unrelated diagnostic call actually helped rule out a broader
auth/scope problem, since jobs/clusters endpoints returned clean, valid —
if empty — responses). The real explanation: Phase 1's original PySpark
activity was run as **interactive notebook cell execution**, which the
Jobs API never tracks — it only tracks runs of registered **Job** objects
(Databricks Workflows). Ethan created a real Job (Workflows → Create Job,
Notebook task, Serverless compute) and triggered it via "Run now," which
produced real, collectible job-run data on the next collector run — see
`docs/roadmap.md`'s Phase 2 entry for the actual `job_id`/`run_id`/duration
landed. The collector's graceful "no cost estimate" path for
node-type-less runs (`pricing.estimate_cost` returning `cost_usd=None`
rather than guessing) was exercised for real here too, since the Job ran
on serverless compute with no `node_type_id` exposed — a genuinely
live-verified instance of that code path, not a synthetic test case.

AWS, Snowflake, and Databricks are now all fully verified against live
data in the same session (see `docs/roadmap.md`'s Phase 2 entry) — Phase 2
is DONE.
...
- Docker build/run verification is a real, disclosed gap in this phase's
  ...
  each of the three services, which also serves his stated goal for this
  phase: real, hands-on Docker experience.
- The only Databricks data landed so far is one real job run on serverless
  compute with no cost estimate attached (correctly — no node-type data
  exists for serverless runs). Phase 3's forecast/optimization logic should
  expect this to be the common case for this source unless Ethan starts
  running Jobs against provisioned (non-serverless) clusters, which do
  expose `node_type_id` for `pricing.estimate_cost` to use.
